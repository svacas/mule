...
public class ExtensionNamespaceHandler extends NamespaceHandlerSupport
{
    ...
    {
        extensionModel.getOperations().forEach(operationModel -> registerParser(hyphenize(operationModel.getName()), new OperationBeanDefinitionParser(extensionModel, operationModel)));
    }

    private void registerConfigurations(ExtensionModel extensionModel)
    {
        extensionModel.getConfigurations().forEach(configurationModel -> registerParser(configurationModel.getName(), new ConfigurationBeanDefinitionParser(configurationModel)));
    }
